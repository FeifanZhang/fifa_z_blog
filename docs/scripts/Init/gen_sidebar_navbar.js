    ...
    fs.readdirSync(curPath).forEach(el => {
        ...
        if(fs.statSync(curNamePath).isFile())
        {
            if(path.extname(el) == ".md" && el != "README.md" && el.substring(0, 1) != "_"){
                ...
                let relativePath = path.relative(docPath, curNamePath)
...
const docPath = path.dirname(path.dirname(__dirname));
const basePath = path.join(docPath, "Content", "Article");
console.log(basePath)
fs.writeFileSync(path.join(docPath, "_navbar.md"), genNavBar(basePath).join('\n'), (err) => {
    if (err) console.log(`error when generate _navbar.md.bak : ${err}`);
})
console.log("gen _navbar finished")

fs.writeFileSync( path.join(docPath, "_sidebar.md" ), genSideBar(basePath).join('\n'), (err) => {
    if (err) console.log(`error when generate _sidebar.md : ${err}`);
})
